...
DECLARE
	...
	function boardtostring(a_board) return varchar2 is
	v_str varchar2(36);
	begin
		for i in 0..5 loop
			for j in 0..5 loop
...
--	  dbms_output.put_line(a_x||a_y||a_block.orientation);
  ...
  v_grid_size number := 6;
	...
	function move(a_position in out blocksArrayType, a_board in out gridtype, a_block_id number, a_step number) returns boolean is
	canmove boolean;
	begin
		if a_step = 0 then
			return true;

...
//global variables
        ...
        function init() {
						...
						document.oncontextmenu = function(){
                                      window.status = "window.event.srcElement.raphaelid = " + window.event.srcElement.raphaelid; 
                                      return false;
                                    };
				...
				function mUp(e) {
					if (e.which == 3) {
						...
						if (canPlace(dummy, dummy.cx, dummy.cy)) {
							block = dummy.clone();
							block.orientation = dummy.orientation.valueOf();
							block.size = dummy.size.valueOf();
							block.marker = marker();
							markers[block.marker] = 1;
							block.cx = dummy.cx.valueOf();
							block.cy = dummy.cy.valueOf();
							block.unmousedown;
							block.unmouseup;
							blocks.push(block);
							doPlace(block, block.cx, block.cy, true);
							placeDummyAtCell(dummy.cx, dummy.cy, true);
							if (dummy.cy == 2 && dummy.orientation == 'h') setXBlock();
						} else {

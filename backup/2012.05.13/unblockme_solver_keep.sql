...
*/
	...
	begin
		...
		if block.orientation = '-' then
			if a_coord =  block.x then	return (not steps.exists(boardtostring(a_board))); end if;

			for i in least(block.x, a_coord)..greatest(block.x, a_coord) + block.length - 1 loop
				if not a_board(i)(block.y) in ('0',block.marker) then return false; end if;	
			end loop;
			...
			if a_coord =  block.y then	return (not steps.exists(boardtostring(a_board))); end if;

			for i in least(block.y, a_coord)..greatest(block.y, a_coord) + block.length - 1 loop
				if not a_board(block.x)(i) in ('0',block.marker) then return false; end if;	
			end loop;
			removeblock(a_board, block);
			block.y := a_coord;
			addblock(a_board, block);
		...
		if steps.exists(boardstring) then
			...
			checkboard(a_board, a_position);
--			printstring(boardtostring(a_board));
			return true;
		end if;
...
if v_depth >= 50 then
	raise_application_error(-20000, 'Max depth exceeded.');
end if;
...
v_depth := v_depth + 1;
					if newposition(i).marker = 'X' and newposition(i).x = 4 then 
						solutionfound;
--						printstring(boardtostring(newboard));
